...
class DisguiseApp extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.api.ApplicationV2) {
  ...
  static PARTS = {
    content: {
      template: "modules/pf2e-ex-token-pack/data/templates/Disguise.hbs",
        ...
        buttons: [
            {
                ...
                callback: async (event, button, dialog) => {
                    ...
                    if (isFullCopy) {
                        ...
                        const originalVisuals = this.actor.getFlag('pf2e-ex-token-pack', 'data_original_visuals');
                    ...
                    const masterList = this.actor.getFlag('pf2e-ex-token-pack', 'disguises') || [];
                    ...
                    await this.actor.setFlag('pf2e-ex-token-pack', flagKey, dataToSave);
                    await this.actor.setFlag('pf2e-ex-token-pack', 'disguises', masterList);
...
class PhaseManagerApp extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.api.ApplicationV2) {
    ...
    async _prepareContext(options) {
        ...
        let masterList = this.actor.getFlag("pf2e-ex-token-pack", "disguises") ?? [];
        const activeDisguiseId = this.actor.getFlag("pf2e-ex-token-pack", "lastAppliedDisguiseId");
        ...
        for (const disguise of [...partition.fulls, ...partition.hybrids]) {
          ...
          const data = this.actor.getFlag("pf2e-ex-token-pack", flagKey);
          if (data) {
    ...
    async _onApplyPhase(event) {
        ...
        const disguise = (this.actor.getFlag("pf2e-ex-token-pack", "disguises") ?? []).find(d => d.id === id);
    ...
    const savedData = this.actor.getFlag('pf2e-ex-token-pack', flagKey);
    if (!savedData) return;
    ...
    tempSheet.close = async (options) => {
        ...
        await this.actor.setFlag('pf2e-ex-token-pack', flagKey, updatedSource);
        await this.actor.setFlag('pf2e-ex-token-pack', 'disguises', masterList);
        ...
        new foundry.applications.api.DialogV2({
            ...
            submit: async (newName) => {
                if (typeof newName === "string" && newName.trim()) {
                    ...
                    await this.actor.setFlag('pf2e-ex-token-pack', flagKey, data);
                    await this.actor.setFlag('pf2e-ex-token-pack', 'disguises', masterList);
                    this.render(true);
                }
            }
    ...
    async _onDeletePhase(event) {
        ...
        await this.actor.setFlag('pf2e-ex-token-pack', 'disguises', masterList);
        await this.actor.unsetFlag('pf2e-ex-token-pack', flagKey);
        this.render(true);
    }
}
...
class DisguiseHudApp extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.api.ApplicationV2) {
    ...
    async _prepareContext(options) {
        ...
        if (originalInList) {
            const originalData = this.actor.getFlag('pf2e-ex-token-pack', 'data_original_visuals');
            if (originalData) {
                original = {
                    id: 'original',
                    type: 'hybrid',
        ...
        for (const d of masterList.filter(d => d.id !== 'original')) {
            const data = this.actor.getFlag('pf2e-ex-token-pack', `data_${d.id}`);
            if (!data) continue;
            const item = {
                ...d,
                name: data.name,
    ...
    _reposition() {
        ...
        const hudPosition = game.settings.get('pf2e-ex-token-pack', 'hudPosition') || 'top-right';
...
class DisguiseAutomationApp extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.api.ApplicationV2) {
  ...
  async _prepareContext(options) {
    const ctx = await super._prepareContext(options);
    let automations = this.actor.getFlag("pf2e-ex-token-pack", "automations") || [];
    const masterDisguiseList = this.actor.getFlag("pf2e-ex-token-pack", "disguises") || [];
    ...
    for (const disguise of masterDisguiseList) {
      ...
      let disguiseData = disguise.isOriginal
        ? this.actor.getFlag("pf2e-ex-token-pack", "data_original_visuals")
        : this.actor.getFlag("pf2e-ex-token-pack", `data_${disguise.id}`);
  ...
  async _onSaveAutomation(event) {
    ...
    await this.actor.setFlag("pf2e-ex-token-pack", "automations", automations);
    this.render(true);
  }
...
class DisguiseMainMenu extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.api.ApplicationV2) {
    ...
    static PARTS = {
        content: { template: "modules/pf2e-ex-token-pack/data/templates/Disguise.hbs" }
    };

    async _prepareContext(options) {
        const ctx = await super._prepareContext(options);
        const highlightDisabled = this.actor.getFlag('pf2e-ex-token-pack', 'highlightDisabled') ?? false;
    ...
    async _onButtonClick(action) {
        ...
        const highlightDisabled = actor.getFlag('pf2e-ex-token-pack', 'highlightDisabled') ?? false;
        ...
        switch (action) {
            ...
            case 'highlight':
                await actor.setFlag('pf2e-ex-token-pack', 'highlightDisabled', !highlightDisabled);
...
async function applyFullDisguise(targetActor, sourceData, targetToken = null, sheet = null, options = { applySize: true }, disguiseId = null) {
    ...
    const activeMode = targetActor.getFlag('pf2e-ex-token-pack', 'active_mode');

    if (activeMode !== 'full') {
        const stateToSave = targetActor.toObject();
        delete stateToSave.flags['pf2e-ex-token-pack'];
        await targetActor.setFlag('pf2e-ex-token-pack', 'last_hybrid_state', stateToSave);
    }

    const sourceClone = foundry.utils.deepClone(sourceData);
    const originalVisuals = targetActor.getFlag('pf2e-ex-token-pack', 'data_original_visuals');
    ...
    if (targetToken) {
        ...
        if (options.applySize) {
            ...
            const originalVisualsToken = targetActor.getFlag('pf2e-ex-token-pack', 'data_original_visuals')?.prototypeToken;
    ...
    await targetActor.setFlag('pf2e-ex-token-pack', 'active_mode', 'full');
    await targetActor.setFlag('pf2e-ex-token-pack', 'lastAppliedDisguiseId', disguiseId);
...
async function applyVisualDisguise(targetActor, visualData, targetToken = null, sheet = null, options = { applySize: true }, disguiseId = null) {
    if (targetActor.getFlag('pf2e-ex-token-pack', 'active_mode') === 'full') {
        let lastHybridState = targetActor.getFlag('pf2e-ex-token-pack', 'last_hybrid_state');
        if (lastHybridState) {
            const mechanicalItemTypes = ['melee', 'spellcastingEntry', 'spell', 'action', 'feat', 'lore', 'skill'];
            const inventoryItemTypes = ['weapon', 'armor', 'shield', 'equipment', 'consumable', 'treasure', 'backpack'];
            ...
            await targetActor.setFlag('pf2e-ex-token-pack', 'last_hybrid_state', syncedHybridState);
    ...
    await targetActor.setFlag('pf2e-ex-token-pack', 'lastAppliedDisguiseId', disguiseId);
    await targetActor.setFlag('pf2e-ex-token-pack', 'active_mode', 'hybrid');
    if (sheet?.rendered) setTimeout(() => sheet.render(true), 100);
    refreshDisguiseApps(targetActor);
}
...
async function _performReset(actor) {
    const hiddenBackup = actor.getFlag("pf2e-ex-token-pack", "data_original");
    if (!hiddenBackup) {
    ...
    await actor.setFlag('pf2e-ex-token-pack', 'data_original_visuals', restoredVisualOriginal);
    const masterList = actor.getFlag('pf2e-ex-token-pack', 'disguises') || [];
    ...
    await actor.setFlag('pf2e-ex-token-pack', 'disguises', masterList);
...
async function saveOriginalState(actor, token) {
    if (actor.getFlag('pf2e-ex-token-pack', 'data_original')) return;
    ...
    await actor.setFlag('pf2e-ex-token-pack', 'data_original', originalVisualData);
    await actor.setFlag('pf2e-ex-token-pack', 'data_original_visuals', originalVisualData);

    let masterList = (actor.getFlag('pf2e-ex-token-pack', 'disguises') || []).filter(d => d.id !== 'original');
    masterList.unshift({
        id: 'original',
        name: originalSource.name,
        type: 'hybrid',
    ...
    await actor.setFlag('pf2e-ex-token-pack', 'disguises', masterList);
    await actor.setFlag('pf2e-ex-token-pack', 'active_mode', 'hybrid');
}
...
async function syncDisguiseProperties(actor, sourcePrototypeToken) {
    ...
    for (const disguise of masterList) {
        const flagKey = disguise.id === 'original' ? 'data_original_visuals' : `data_${disguise.id}`;
        const originalDisguiseData = actor.getFlag("pf2e-ex-token-pack", flagKey);
        ...
        if (dataChanged) {
            updates[`flags.pf2e-ex-token-pack.${flagKey}`] = targetDisguiseData;
        }
    ...
    if (masterListChanged) {
        updates['flags.pf2e-ex-token-pack.disguises'] = masterList;
    }
...
async function requestDisguiseChange(request) {
    if (game.user.isGM) {
        ...
        if (request.sourceType === 'actor') {
            ...
            sourceData = authoritativeActor.getFlag('pf2e-ex-token-pack', flagKey);
        }
        ...
        game.socket.emit('module.pf2e-ex-token-pack', {
...
async function enhanceTokenConfigWindow(app, html) {
    ...
    const activeDisguiseId = actor.getFlag("pf2e-ex-token-pack", "lastAppliedDisguiseId");
    ...
    if (appearanceTab.length) {
        ...
        const disguiseData = actor.getFlag("pf2e-ex-token-pack", flagKey);
    ...
    footer.find(".update-disguise").on("click", async (event) => {
        ...
        const flagKey = activeDisguiseId === 'original' ? 'data_original_visuals' : `data_${activeDisguiseId}`;
        const oldDisguiseData = foundry.utils.deepClone(actor.getFlag("pf2e-ex-token-pack", flagKey));
        ...
        await actor.setFlag("pf2e-ex-token-pack", flagKey, oldDisguiseData);
        ui.notifications.info(game.i18n.format("Disguise.UpdateSuccess", { name: oldDisguiseData.name }));
        
        const disguiseToReapply = (actor.getFlag("pf2e-ex-token-pack", "disguises") ?? []).find(d => d.id === activeDisguiseId);
...
Hooks.on("renderTokenHUD", (hud, html, data) => {
    ...
    buttonElement.addEventListener('contextmenu', (ev) => {
        ...
        ev.stopPropagation();
        document.getElementById("disguise-hud-app")?.remove();
        if (!actor.getFlag('pf2e-ex-token-pack', 'disguises')?.some(d => !d.isOriginal)) {
...
Hooks.on("refreshToken", (token) => {
    ...
    const hasCustomDisguises = actor.getFlag('pf2e-ex-token-pack', 'disguises')?.some(d => !d.isOriginal);
    const highlightDisabled = actor.getFlag('pf2e-ex-token-pack', 'highlightDisabled') ?? false;

    if (hasCustomDisguises && !highlightDisabled) {
        const marker = new PIXI.Graphics();
        marker.name = "disguiseMarker";
        const colorSetting = game.settings.get('pf2e-ex-token-pack', 'highlightColor') || "#9400D3";
        const borderColor = parseInt(colorSetting.replace("#", ""), 16);
        const borderWidth = 4;
        marker.lineStyle(borderWidth, borderColor, 1, 0);

        const style = game.settings.get('pf2e-ex-token-pack', 'highlightStyle') || 'rounded-rect';
...
Hooks.on("renderSettingsConfig", (app, html) => {
    const colorInput = html.querySelector('[name="pf2e-ex-token-pack.highlightColor"]');
...
Hooks.on("createItem", async (item, options, userId) => {
    ...
    const automations = actor.getFlag('pf2e-ex-token-pack', 'automations') || [];
    ...
    const revertDisguiseId = actor.getFlag('pf2e-ex-token-pack', 'lastAppliedDisguiseId') || 'original';
    await item.setFlag('pf2e-ex-token-pack', 'revertDisguiseId', revertDisguiseId);
    
    const disguiseToApply = (actor.getFlag('pf2e-ex-token-pack', 'disguises') || []).find(d => d.id === rule.disguiseId);
...
Hooks.on("deleteItem", async (item, options, userId) => {
    if (game.userId !== userId || item.type !== 'effect' || !item.actor) return;
    const revertDisguiseId = item.getFlag('pf2e-ex-token-pack', 'revertDisguiseId');
    if (!revertDisguiseId) return;

    const actor = item.actor;
    const disguiseToRevert = (actor.getFlag('pf2e-ex-token-pack', 'disguises') || []).find(d => d.id === revertDisguiseId);
...
Hooks.on('init', () => {
    game.settings.register('pf2e-ex-token-pack', 'highlightColor', {
    ...
    game.settings.register('pf2e-ex-token-pack', 'highlightStyle', {
    ...
    game.settings.register('pf2e-ex-token-pack', 'hudPosition', {
    ...
    game.socket.on('module.pf2e-ex-token-pack', async (data) => {
        ...
        const flagKey = data.sourceId === 'original'
            ...
            : `data_${data.sourceId}`;
        const sourceData = targetActor.getFlag('pf2e-ex-token-pack', flagKey);
...
Hooks.on("updateToken", async (tokenDoc, changes, options, userId) => {
    ...
    if (!actor || actor.type !== 'character' || !tokenDoc.actorLink) {
        ...
        if (authoritativeActor?.isOwner && (authoritativeActor.getFlag("pf2e-ex-token-pack", "disguises") || []).length > 0) {
...
Hooks.on("updateActor", async (actor, changes, options, userId) => {
    ...
    if ((actor.getFlag("pf2e-ex-token-pack", "disguises") || []).length > 0) {
