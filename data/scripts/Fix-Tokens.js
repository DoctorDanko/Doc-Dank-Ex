...
class MainMenuForm extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.api.ApplicationV2) {
    static PARTS = {
        content: { template: "modules/pf2e-ex-token-pack/data/templates/Fix-Tokens.hbs" }
    };
...
class ExcludeTypesForm extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.api.ApplicationV2) {
    ...
    static get DEFAULT_OPTIONS() {
        return {
            ...
            classes: ["pf2e-ex-token-pack", "exclusion-form"],
    ...
    async _prepareContext(options) {
        const ctx = await super._prepareContext(options);
        const saved = game.settings.get('pf2e-ex-token-pack', this.setting);
        ...
        html.find('button.form-submit').on('click', async (event) => {
            ...
            if (form) {
                try {
                    ...
                    await game.settings.set('pf2e-ex-token-pack', this.setting, selectedIds);
                } catch (e) {
...
class ExcludeActorsForm extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.api.ApplicationV2) {
    ...
    buildActorTree() {
        let saved = game.settings.get('pf2e-ex-token-pack', this.setting);
        const defaultSet = game.settings.get('pf2e-ex-token-pack', 'actor-default-set');
        ...
        html.find('button.form-submit').on('click', async (event) => {
            ...
            if (form) {
                try {
                    ...
                    const selectedIds = Object.keys(formData).filter(key => formData[key]);
                    await game.settings.set('pf2e-ex-token-pack', this.setting, selectedIds);
                    if (!game.settings.get('pf2e-ex-token-pack', 'actor-default-set')) {
                        await game.settings.set('pf2e-ex-token-pack', 'actor-default-set', true);
                    }
                } catch (e) {
...
function isActorExcludedBySettings(actor) {
    const excludedTypes = game.settings.get('pf2e-ex-token-pack', 'fix-tokens-excluded-types');
...
async function runUpdateScript(reportData) {
    const manualExcludedIds = game.settings.get('pf2e-ex-token-pack', 'fix-tokens-manual-excluded-actors');

    for (const actor of game.actors.contents) {
        if (foundry.utils.hasProperty(actor.flags, 'pf2e-ex-token-pack')) continue;
        ...
        for (const scene of game.scenes) {
            for (const tokenDoc of scene.tokens.filter(t => t.actorId === actor.id)) {
                if (foundry.utils.hasProperty(tokenDoc.flags, 'pf2e-ex-token-pack') || foundry.utils.hasProperty(tokenDoc.delta?.flags, 'pf2e-ex-token-pack')) continue;
...
async function runUpdateDisguisesForActors(reportData) {
    for (const actor of game.actors.contents) {
        ...
        for (const disguise of masterList) {
            ...
            const savedData = actor.getFlag('pf2e-ex-token-pack', flagKey);
            ...
            if (needsUpdate) {
                updates[`flags.pf2e-ex-token-pack.${flagKey}`] = updatedDisguiseData;
        ...
        if (actorDataChanged) {
            ...
            if (activeDisguiseId && updatedDisguiseIds.has(activeDisguiseId)) {
                const activeDisguise = (actor.getFlag('pf2e-ex-token-pack', 'disguises') || []).find(d => d.id === activeDisguiseId);
                if (activeDisguise) {
                    const flagKey = activeDisguiseId === 'original' ? 'data_original_visuals' : `data_${activeDisguiseId}`;
                    const newVisualData = actor.getFlag('pf2e-ex-token-pack', flagKey);
                    if (newVisualData) {
...
async function runUpdateDisguisesForTokens(reportData) {
    for (const scene of game.scenes) {
        for (const tokenDoc of scene.tokens) {
            const tokenFlags = foundry.utils.getProperty(tokenDoc, "delta.flags.pf2e-ex-token-pack");
            ...
            for (const disguise of updatedMasterList) {
                ...
                if (needsUpdate) {
                    tokenUpdatePayload[`delta.flags.pf2e-ex-token-pack.${flagKey}`] = updatedDisguiseData;
            ...
            if (tokenDataChanged) {
                tokenUpdatePayload['delta.flags.pf2e-ex-token-pack.disguises'] = updatedMasterList;
                await tokenDoc.update(tokenUpdatePayload);

                const tokenActiveDisguiseId = tokenDoc.actor?.getFlag('pf2e-ex-token-pack', 'lastAppliedDisguiseId');
                if (tokenActiveDisguiseId && updatedDisguiseIds.has(tokenActiveDisguiseId)) {
                    ...
                    if (activeDisguise) {
                        ...
                        const newVisualData = tokenDoc.actor.getFlag('pf2e-ex-token-pack', flagKey);
...
Hooks.once("init", () => {
    game.settings.register("pf2e-ex-token-pack", "fix-tokens-excluded-types", { 
        scope: "world", 
        config: false, 
        type: Array, 
    ...
    game.settings.register("pf2e-ex-token-pack", "fix-tokens-manual-excluded-actors", { 
        scope: "world", 
        config: false, 
        type: Array, 
        default: [] 
    ...
    game.settings.register("pf2e-ex-token-pack", "actor-default-set", { 
        scope: "world", 
        config: false, 
        type: Boolean, 
        default: false 
    ...
    game.settings.registerMenu("pf2e-ex-token-pack", "myScriptMenu", { 
