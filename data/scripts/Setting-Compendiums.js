...
class PathfinderBestiaryTokenPack {
  static MASTER_FILE_PATH = "modules/pf2e-ex-token-pack/data/_sources/bestiaries-master.json";
  static SOURCE_FOLDER_PATH = "modules/pf2e-ex-token-pack/data/_sources";
  static USER_BESTIARY_PATH = "modules/pf2e-ex-token-pack";
  ...
  static registerSettings() {
    this.KEYS.forEach(({key, name, hint}) => {
      game.settings.register("pf2e-ex-token-pack", `enableOverwrite${key}`, {
  ...
  static async buildUserBestiary(showReloadPrompt = true) {
    try {
        ...
        for (const [key, content] of Object.entries(masterData)) {
            ...
            if (!game.settings.get("pf2e-ex-token-pack", settingKey)) {
        ...
        try {
            ...
            console.log(`pf2e-ex-token-pack | ${game.i18n.localize("SettingCompendiums.LogUserBestiaryNew")}`);
        }
        ...
        if (idealDataString === currentDataString) {
            console.log(`pf2e-ex-token-pack | ${game.i18n.localize("SettingCompendiums.LogUserBestiaryUpToDate")}`);
            return null;
        }

        console.log(`pf2e-ex-token-pack | ${game.i18n.localize("SettingCompendiums.LogUserBestiaryRebuilding")}`);
        ...
        console.log(`pf2e-ex-token-pack | ${game.i18n.localize("SettingCompendiums.LogUserBestiaryRebuilt")}`);
        ...
        const content = await foundry.applications.handlebars.renderTemplate("modules/pf2e-ex-token-pack/data/templates/Setting-Compendiums.hbs", { isReloadDialog: true });
        ...
        console.error(`pf2e-ex-token-pack | ${game.i18n.localize("SettingCompendiums.BestiariesSyncError")}:`, error);
...
class PathfinderBestiarySettingsMenu extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.api.ApplicationV2) {
  ...
  static get DEFAULT_OPTIONS() {
    return foundry.utils.mergeObject(super.DEFAULT_OPTIONS, {
      id: "pf2e-ex-token-pack-settings",
      window: {
  ...
  static PARTS = {
    content: {
      template: "modules/pf2e-ex-token-pack/data/templates/Setting-Compendiums.hbs",
      classes: ["pf2e-ex-token-pack-settings-form-content"]
    },
  };
  ...
  async _prepareContext(options) {
    ...
    for (const { key, name, hint, category = "bestiaries" } of PathfinderBestiaryTokenPack.KEYS) {
      const settingKey = `enableOverwrite${key}`;
      const value = game.settings.get("pf2e-ex-token-pack", settingKey);
  ...
  async _onFormSubmit(event) {
    ...
    for (const [key, value] of Object.entries(formData)) {
      if (key.startsWith("enableOverwrite")) {
        const current = game.settings.get("pf2e-ex-token-pack", key);
        if (current !== value) {
          changed = true;
          await game.settings.set("pf2e-ex-token-pack", key, value);
        }
      }
    }
...
class BestiaryIntegrityChecker {
  ...
  async runCheck() {
    try {
      ...
      const collectPath = (p) => {
        if (p) {
          ...
          if (dir.startsWith("modules/pf2e-ex-token-pack")) relatedDirs.add(dir);
        }
      };
      ...
      if (noProblems) {
        ...
        const content = await foundry.applications.handlebars.renderTemplate("modules/pf2e-ex-token-pack/data/templates/Setting-Compendiums.hbs", { 
    ...
    if (hasUnusedFiles) {
        ...
        const modulePath = "modules/pf2e-ex-token-pack";
    ...
    if (masterChanged || orderChanged) {
        ...
        console.log("pf2e-ex-token-pack | JSON не изменился, загрузка пропущена.");
    }
...
class BestiaryCheckResultsDialog extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.api.ApplicationV2) {
  ...
  static get DEFAULT_OPTIONS() {
    return foundry.utils.mergeObject(super.DEFAULT_OPTIONS, {
      id: "pf2e-ex-token-pack-check-results",
      window: {
  ...
  async _postRender(context, options) {
    ...
    if (!resolver) {
      console.error("pf2e-ex-token-pack | Resolver function not found!");
      return;
    }
...
Hooks.once('init', () => {
  ...
  game.settings.registerMenu("pf2e-ex-token-pack", "tokenPackSettings", {
